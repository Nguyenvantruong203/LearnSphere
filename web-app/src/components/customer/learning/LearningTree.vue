<template>
    <div class="space-y-5">
        <div v-for="(topic, tIndex) in topics" :key="topic.id"
            ...
            <LearningItem :title="topic.title" :expandable="true" :expanded="expandedTopics.includes(topic.id)"
                @click="toggleTopic(topic.id)" class="topic-header">
                <template #icon>
                    <div
                        class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md transition-transform">
                ...
                <div v-if="expandedTopics.includes(topic.id)" class="space-y-2 p-3">
                    ...
                    <div v-for="(lesson, lIndex) in topic.lessons" :key="lesson.id">
                        <LearningItem :title="lesson.title" :expandable="!!lesson.quiz"
                            ...
                            <template #icon>
                                <svg class="w-6 h-6 text-client flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <template #extra>
                                <span v-if="lesson.quiz"
                                    class="ml-2 px-2 py-0.5 text-xs font-medium rounded bg-indigo-100 text-indigo-700">
                                    Quiz
                                </span>
                            </template>
                        </LearningItem>

                        <!-- ✅ Quiz nằm ngay dưới bài học -->
                        <div v-if="lesson.quiz && expandedLessons.includes(lesson.id)" data-no-bubble
                            class="lesson-quiz bg-lesson-quiz" @click.stop="handleOpenLessonQuiz(lesson)">

                            <div class="flex items-center gap-2 text-indigo-700 font-medium min-w-0 flex-1">
                                <svg class="w-5 h-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="32"
                                ...
                                <span class="truncate block max-w-[220px]" :title="lesson.quiz.title">
                                    {{ lesson.quiz.title || 'Quiz của bài học' }}
                                </span>
                            </div>
                            <span class="text-xs font-semibold text-indigo-600 flex-shrink-0">Làm bài</span>
                        </div>
                    </div>

                    <!-- 🧩 Quiz cuối topic -->
                    <div v-if="topic.quiz" class="topic-quiz bg-topic-quiz" data-no-bubble
                        @click.stop.prevent="handleOpenTopicQuiz(topic)">
                        <div class="flex items-center gap-2 text-purple-800 font-semibold min-w-0 flex-1">
                            <svg class="w-5 h-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                viewBox="0 0 24 24">
                                <path fill="currentColor"
                                    d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2m0 14H8V4h12zm-6.49-5.84c.41-.73 1.18-1.16 1.63-1.8c.48-.68.21-1.94-1.14-1.94c-.88 0-1.32.67-1.5 1.23l-1.37-.57C11.51 5.96 12.52 5 13.99 5c1.23 0 2.08.56 2.51 1.26c.37.6.58 1.73.01 2.57c-.63.93-1.23 1.21-1.56 1.81c-.13.24-.18.4-.18 1.18h-1.52c.01-.41-.06-1.08.26-1.66m-.56 3.79c0-.59.47-1.04 1.05-1.04c.59 0 1.04.45 1.04 1.04c0 .58-.44 1.05-1.04 1.05c-.58 0-1.05-.47-1.05-1.05" />
                            </svg>
                            <span class="truncate block max-w-[220px]" :title="topic.quiz.title">
                                {{ topic.quiz.title || 'Quiz cuối chương' }}
                            </span>
                        </div>
                        <span class="text-sm font-medium text-purple-700 flex-shrink-0">Làm bài</span>
                    </div>
                </div>
            </transition>
        </div>
...
watch([expandedTopics, expandedLessons], ([topics, lessons]) => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ expandedTopics: topics, expandedLessons: lessons }))
}, { deep: true })
...
const toggleLesson = (topicId: number, lessonId: number) => {
    if (expandedLessons.value.includes(lessonId)) {
        expandedLessons.value = expandedLessons.value.filter(id => id !== lessonId)
    } else {
...
.lesson-quiz {
    /* Thêm cursor-pointer */
    @apply flex items-center justify-between h-[46px] rounded-xl bg-indigo-50 cursor-pointer px-4 transition;
    margin-left: 2.5rem;
    margin-top: 0.25rem;
}
...
.topic-quiz {
    /* Thêm cursor-pointer */
    @apply flex items-center justify-between h-[56px] rounded-xl bg-purple-100 cursor-pointer px-4 transition;
}
</style>
