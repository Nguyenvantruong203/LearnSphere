  ...
  <div class="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 to-gray-100">
    ...
    <header class="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 shadow-sm sticky top-0 z-40">
      <div class="px-4 py-6 flex items-center justify-between">
        ...
        <button
          v-if="lesson?.quiz"
          class="group relative px-6 py-3 bg-gradient-to-r from-teal-500 to-blue-500 text-white rounded-2xl text-base font-semibold transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 flex items-center gap-3"
    ...
    <main class="flex-1 overflow-y-auto">
      <div class="max-w-6xl mx-auto p-3 space-y-6">
        ...
        <template v-else>
          ...
          <div
            v-if="lesson.quiz"
            class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-8 text-white md:hidden"
          >
            <div class="flex items-center justify-between">
              ...
              <button
                class="px-6 py-3 bg-white text-indigo-600 rounded-2xl font-semibold"
                @click="$emit('open-quiz', lesson.quiz.id)"
              >
                Bắt đầu
              </button>
...
const emit = defineEmits(['open-quiz', 'lesson-completed'])
...
const loadYouTubePlayer = () => {
  ...
  // Destroy player trước khi load bài học mới
